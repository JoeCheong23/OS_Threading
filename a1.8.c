...
void* merge_sort(void *data_block) {
    ...
    if ((block->size > SPLIT)) {
        ...
        if (process_count < MAX_NUM_OF_PROCESS) {
            ...
            if (pid != 0) { //if parent, sort right block, then merge
            ...
            } else { //child sorts left side then gives the sorted array to parent
